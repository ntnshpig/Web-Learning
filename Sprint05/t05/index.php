<?php 
    class StrFrequency {
        ...
        public function letterFrequencies() {
            $str1 = count_chars($this->str, 1);
            $res = array();
            foreach($str1 as $i => $val){
                if(ctype_alpha($i)){
                    $res[strtoupper(chr($i))] = $val;
                }
            }
            ksort($res);
            return $res;
        ...
        public function wordFrequencies() {
            $res = preg_replace('/[^a-z\s]/i', '', $this->str); 
            return array_count_values(str_word_count(strtoupper($res) , 1));
        }
        public function reverseString() {
            $res = preg_replace('/[^a-z ]/i', ' ', $this->str); 
            return strrev($res);
        }
    }
